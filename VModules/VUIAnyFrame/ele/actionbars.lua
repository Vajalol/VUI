...
-- VUIAnyFrame Element: ActionBars
local AddonName, VUI = ...
local M = _G["VUIAnyFrame"]
local L = M.L

-- ActionBar specific functions
local function MAActionBar_OnLoad()
	-- Hook action bar updates
	if not M:IsHooked("ActionBarController_UpdateAll") then
		M:SecureHook("ActionBarController_UpdateAll", function()
			C_Timer.After(0.1, function()
				if M:GetCurrentEle() == "ActionBars" then
					-- Update action bar settings when Blizzard updates bars
					M:UpdateActionBars()
				end
			end)
		end)
	...
	-- Action bar element options
	if not M.GetCurrentEle or M:GetCurrentEle() ~= "ActionBars" then
		return
	end
	
	-- Refresh action bar frames
	local frame = M:GetEleFrames()["ActionBars"]
	if not frame then
		return
	end
	
	-- Create options for action bars
	local parent = _G["VUIAnyFrameActionBarsOptions"] or CreateFrame("Frame", "VUIAnyFrameActionBarsOptions")
	parent:SetSize(600, 600)
	
	-- Action bars scale option
	local y = -40
	M:CreateSlider(parent, 20, y, "ActionBars", "scale", 1, 0.1, 0.1, 4, function()
		M:UpdateActionBars()
	end)
	
	-- Action bars spacing option
	y = y - 40
	M:CreateSlider(parent, 20, y, "ActionBars", "spacing", 2, 1, 0, 40, function()
		M:UpdateActionBars()
	end)
	
	-- Action bars columns option
	y = y - 40
	M:CreateSlider(parent, 20, y, "ActionBars", "columns", 12, 1, 1, 12, function()
		M:UpdateActionBars()
	end)
	
	-- Action bars padding option
	y = y - 40
	M:CreateSlider(parent, 20, y, "ActionBars", "padding", 0, 1, 0, 20, function()
		M:UpdateActionBars()
	end)
	...
	resetButton:SetPoint("TOPLEFT", parent, "TOPLEFT", 20, y)
	resetButton:SetText("Reset")
	resetButton:SetScript("OnClick", function()
		M:ResetFrameSettings("ActionBars")
		M:UpdateActionBars()
	end)
	
	-- Store for reference
	M.actionBarsOptions = parent
end

-- Register the ActionBars element
function M:RegisterActionBars()
	-- Skip if already registered
	if M:GetEleFrames()["ActionBars"] then
		return
	end
	
	-- Create a proxy frame for action bars
	local actionBarsProxy = CreateFrame("Frame", "VUIAnyFrameActionBarsProxy", UIParent)
	actionBarsProxy:SetSize(500, 40)
	actionBarsProxy:SetPoint("BOTTOM", UIParent, "BOTTOM", 0, 120)
	
	-- Register the proxy frame
	M:RegisterWidget(actionBarsProxy, "ActionBars", "Action Bars")
...
-- Update action bars based on settings
function M:UpdateActionBars()
	...
	-- Get settings
	local scale = M:GetEleOption("ActionBars", "scale", 1)
	local spacing = M:GetEleOption("ActionBars", "spacing", 2)
	local columns = M:GetEleOption("ActionBars", "columns", 12)
	local padding = M:GetEleOption("ActionBars", "padding", 0)
	
	-- Apply settings to action bar frames
	if MainMenuBar then
		MainMenuBar:SetScale(scale)
	end
	
	-- Apply to action bars
	local actionBars = {
		"MultiBarBottomLeft",
		"MultiBarBottomRight",
		"MultiBarRight",
	...
	for i = 5, 7 do
		...
		if multiBar then
			table.insert(actionBars, "MultiBar" .. i)
		end
	end
	
	-- Apply settings to each action bar
	for _, barName in ipairs(actionBars) do
		local bar = _G[barName]
		if bar then
			bar:SetScale(scale)
			
			-- Adjust button layout if possible
			if not InCombatLockdown() then
				-- Only adjust if possible without taint
				local isMain = (barName == "MainMenuBar")
				local numButtons = isMain and NUM_ACTIONBAR_BUTTONS or 12
				
				-- Adjust the button layout if accessible
				if bar.SetNumButtons and bar.SetButtonSize then
					bar:SetNumButtons(numButtons)
					bar:SetButtonSize(36 + padding, 36 + padding)
				end
				...
				for i = 1, numButtons do
					...
					if button and i > 1 then
						local prevButton = _G[barName .. "Button" .. (i-1)]
						if prevButton then
							-- Apply spacing based on column settings
...
-- Register in the module's loading
M:RegisterActionBars()
