...
local TabPanelMethods = {
	...
	DrawButtons = function(self)
		...
		for _, tab in pairs(self.tabs) do
			...
			if not btn then
				btn = self.VUIConfig:Button(btnContainer, nil, self.buttonHeight);
				tab.button = btn;
				btn.tabFrame = self;
			...
			if self.vertical then
				...
				self.VUIConfig:ButtonAutoWidth(btn);
			end

			if self.vertical then
				if not prevBtn then
					self.VUIConfig:GlueTop(btn, btnContainer, 0, 0, 'CENTER');
				else
					self.VUIConfig:GlueBelow(btn, prevBtn, 0, -1);
				end
			else
				if not prevBtn then
					self.VUIConfig:GlueTop(btn, btnContainer, 0, 0, 'LEFT');
				else
					self.VUIConfig:GlueRight(btn, prevBtn, 5, 0);
				end
			end
	...
	DrawFrames = function(self)
		for _, tab in pairs(self.tabs) do
			if not tab.frame then
				tab.frame = self.VUIConfig:Frame(self.container);
			end
			...
			if tab.layout then
				self.VUIConfig:BuildWindow(tab.frame, tab.layout);
				self.VUIConfig:EasyLayout(tab.frame, { padding = { top = 10, left = 5, right = 5 } });
