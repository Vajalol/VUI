...
local dropdowns = VUIConfigDropdowns or {};
VUIConfigDropdowns = dropdowns;
...
local DropdownMethods = {
	buttonCreate = function(parent)
		...
		if dropdown.multi then
			optionButton = dropdown.VUIConfig:Checkbox(parent, '', parent:GetWidth(), 20);
		else
			optionButton = dropdown.VUIConfig:HighlightButton(parent, parent:GetWidth(), 20, '');
...
--- }
--- @return Dropdown
function VUIConfig:Dropdown(parent, width, height, options, value, multi, assoc)
	--- @class Dropdown
	local dropdown = self:Button(parent, width, height, '');
	dropdown.VUIConfig = self;
