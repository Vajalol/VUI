...
VUIConfig.ContextMenuMethods = {
	...
	CreateItem        = function(parent, data, i)
		...
		if data.title then
			itemFrame = parent.VUIConfig:Frame(parent, nil, 20);
			itemFrame.text = parent.VUIConfig:Label(itemFrame);
			parent.VUIConfig:GlueLeft(itemFrame.text, itemFrame, 0, 0, true);
		elseif data.isSeparator then
			itemFrame = parent.VUIConfig:Frame(parent, nil, 20);
			itemFrame.texture = parent.VUIConfig:Texture(itemFrame, nil, 8,
		...
		elseif data.checkbox then
			itemFrame = parent.VUIConfig:Checkbox(parent, '');
		elseif data.radio then
			itemFrame = parent.VUIConfig:Radio(parent, '', data.radioGroup);
		elseif data.text then
			itemFrame = parent.VUIConfig:HighlightButton(parent, nil, 20);
		end
		...
		if not data.isSeparator and data.children then
			itemFrame.icon = parent.VUIConfig:Texture(itemFrame, 10, 10, [[Interface\Buttons\SquareButtonTextures]]);
			itemFrame.icon:SetTexCoord(0.42187500, 0.23437500, 0.01562500, 0.20312500);
			parent.VUIConfig:GlueRight(itemFrame.icon, itemFrame, -4, 0, true);

			itemFrame.childContext = parent.VUIConfig:ContextMenu(parent, data.children, true, parent.level + 1);
	...
	UpdateItem        = function(parent, itemFrame, data, i)
		...
		if data.title then
			...
			parent.VUIConfig:ButtonAutoWidth(itemFrame);
	...
	DrawOptions       = function(self, options)
		...
		local _, totalHeight = self.VUIConfig:ObjectList(
			self,
			self.optionFrames,
			self.CreateItem,
			self.UpdateItem,
...
function VUIConfig:ContextMenu(parent, options, stopHook, level)
	---@class ContextMenu
	local panel = self:Panel(parent);
	panel.VUIConfig = self;
