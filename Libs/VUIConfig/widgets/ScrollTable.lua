	...
	SetColumns        = function(self, columns)
		...
		for i = 1, #columns do
			...
			if not columnHeadFrame.columns[i] then
				columnFrame = self.VUIConfig:HighlightButton(columnHeadFrame);
				columnFrame:SetPushedTextOffset(0, 0);

				columnFrame.arrow = self.VUIConfig:Texture(columnFrame, 8, 8, [[Interface\Buttons\UI-SortArrow]]);
			...
			if align == 'LEFT' then
				...
				self.VUIConfig:GlueRight(columnFrame.arrow, columnFrame, 0, 0, true);
			else
				columnFrame.arrow:ClearAllPoints();
				self.VUIConfig:GlueLeft(columnFrame.arrow, columnFrame, 5, 0, true);
			end
		...
		for i = 1, numberOfRows do
			...
			for j = 1, #self.columns do
				...
				if not cell then
					...
					cell.text = self.VUIConfig:FontString(cell, '');
	...
	DoCellUpdate      = function(table, shouldShow, rowFrame, cellFrame, value, columnData, rowData, rowIndex)
		if shouldShow then
			...
			elseif format == 'money' then
				value = table.VUIConfig.Util.formatMoney(value);
				cellFrame.text:SetText(value);
			elseif format == 'moneyShort' then
				value = table.VUIConfig.Util.formatMoney(value, true);
				cellFrame.text:SetText(value);
			...
			elseif format == 'icon' then
				if cellFrame.texture then
					...
					cellFrame.texture = table.VUIConfig:Texture(cellFrame, iconSize, iconSize, value);
			...
			elseif format == 'color' then
				if cellFrame.texture then
					...
					local iconSize = 10
					cellFrame.texture = table.VUIConfig:Texture(cellFrame, iconSize, iconSize, [[Interface\Buttons\WHITE8X8]])
					cellFrame.texture:SetVertexColor(value.r, value.g, value.b, value.a)
					cellFrame.texture:SetPoint('CENTER', 0, 0)
				end
			elseif format == 'delete' then
				if not cellFrame.texture then
					local iconSize = 15
					cellFrame.texture = table.VUIConfig:Texture(cellFrame, iconSize, iconSize, [[Interface\UITools\UITools]])
			...
			if color then
				...
				table.VUIConfig:SetTextColor(cellFrame.text, 'normal');
			end

			if table.selectionEnabled then
				if table.selected == rowIndex then
					table:SetHighLightColor(rowFrame, table.VUIConfig.config.highlight.color);
				else
