...
local SimpleEditBoxMethods = {
	SetFontSize = function(self, newSize)
		self:SetFont(self:GetFont(), newSize, self.VUIConfig.config.font.effect);
	end
};
...
--- @return EditBox
function VUIConfig:SimpleEditBox(parent, width, height, text)
	--- @type EditBox
	local editBox = CreateFrame('EditBox', nil, parent);
	editBox.VUIConfig = self;
...
function VUIConfig:ApplyPlaceholder(widget, placeholderText, icon, iconColor)
	widget.placeholder = {};
...
function VUIConfig:SearchEditBox(parent, width, height, placeholderText)
...
local EditBoxEvents = {
	...
	OnTextChanged = function(self, isUserInput)
		local value = VUIConfig.Util.stripColors(self:GetText());
...
--- @return EditBox
function VUIConfig:EditBox(parent, width, height, text, validator)
	validator = validator or VUIConfig.Util.editBoxValidator;
...
function VUIConfig:NumericBox(parent, width, height, text, validator)
...
local MoneyBoxMethods = {
	SetValue = function(self, value)
		self.value = value;
		local formatted = self.VUIConfig.Util.formatMoney(value);
...
function VUIConfig:MoneyBox(parent, width, height, text, validator, excludeCopper)
...
function VUIConfig:MultiLineBox(parent, width, height, text, readOnly)
