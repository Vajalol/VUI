...
function VUIConfig:SliderButton(parent, width, height, direction)
...
--- This is only useful for scrollBars not created using VUIConfig
function VUIConfig:StyleScrollBar(scrollBar)
	local buttonUp, buttonDown = scrollBar:GetChildren();

	scrollBar.background = VUIConfig:Panel(scrollBar);
...
function VUIConfig:Slider(parent, width, height, value, vertical, min, max)
...
function VUIConfig:SliderWithBox(parent, width, height, value, min, max)
...
function VUIConfig:ScrollBar(parent, width, height, horizontal)
