		...
		for i = 1, #self.columns do
			...
			if col.header and StringLength(col.header) > 0 then
				if not self.headers[i] then
					self.headers[i] = {
						text = self.VUIConfig:FontString(self, ''),
					};
				end
				...
				self.VUIConfig:GlueTop(column.text, self, marginLeft, 0, 'LEFT');
	...
	DrawData = function(self)
		...
		for y = 1, #self.tableData do
			...
			for x = 1, #self.columns do
				...
				self.VUIConfig:GlueTop(cell.text, self, marginLeft, marginTop, 'LEFT');
				marginLeft = marginLeft + col.width;
			end
...
function VUIConfig:Table(parent, width, height, rowHeight, columns, data)
