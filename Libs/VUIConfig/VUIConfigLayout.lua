...
local EasyLayoutRow = {
	AddElement      = function(self, frame, config)
		if not frame.layoutConfig then
			frame.layoutConfig = VUIConfig.Util.tableMerge(defaultElementConfig, config or {});
		elseif config then
			frame.layoutConfig = VUIConfig.Util.tableMerge(frame.layoutConfig, config or {});
		end
	...
	AddElements     = function(self, ...)
		...
		for i = 1, #r do
			self:AddElement(r[i], VUIConfig.Util.tableMerge(defaultElementConfig, cfg));
		end
	end,
...
--@param parent Frame
--@param config table
function VUIConfig:EasyLayoutRow(parent, config)
	---@class EasyLayoutRow
	local row = {
		parent   = parent,
...
local EasyLayout = {
	...
	AddRow = function(self, config)
		...
		local row = self.VUIConfig:EasyLayoutRow(self, config);
...
function VUIConfig:EasyLayout(parent, config)
	parent.VUIConfig = self;
