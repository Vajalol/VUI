local MAJOR, MINOR = 'VUIConfig', 10;
--- @class VUIConfig
local VUIConfig = LibStub:NewLibrary(MAJOR, MINOR);
...
VUIConfig.moduleVersions = {};
if not VUIConfigInstances then
	VUIConfigInstances = {VUIConfig};
else
	TableInsert(VUIConfigInstances, VUIConfig);
end

function VUIConfig:NewInstance()
	local instance = CopyTable(self);
	instance:ResetConfig();
	TableInsert(VUIConfigInstances, instance);
	return instance;
end

function VUIConfig:RegisterModule(module, version)
	self.moduleVersions[module] = version;
end

function VUIConfig:UpgradeNeeded(module, version)
...
function VUIConfig:RegisterWidget(name, func)
...
function VUIConfig:InitWidget(widget)
...
function VUIConfig:SetObjSize(obj, width, height)
	if width then
		obj:SetWidth(width);
	end
...
function VUIConfig:SetTextColor(fontString, colorType)
...
VUIConfig.SetHighlightBorder = function(self)
	if self.target then
		self = self.target;
	end
	...
	local hc = self.VUIConfig.config.highlight.color;
...
VUIConfig.ResetHighlightBorder = function(self)
	if self.target then
		self = self.target;
	end
...
function VUIConfig:HookHoverBorder(object)
...
function VUIConfig:ApplyBackdrop(frame, type, border, insets)
...
function VUIConfig:ClearBackdrop(frame)
	if not frame.SetBackdrop then
		Mixin(frame, BackdropTemplateMixin)
	end
...
function VUIConfig:ApplyDisabledBackdrop(frame, enabled)
...
function VUIConfig:HookDisabledBackdrop(frame)
	local this = self;
...
function VUIConfig:StripTextures(frame)
...
function VUIConfig:MakeDraggable(frame, handle)
...
function VUIConfig:MakeResizable(frame, direction)
