local Debuffs = VUI:NewModule("Buffs.Debuffs");

function Debuffs:OnEnable()
    if C_AddOns.IsAddOnLoaded("BlizzBuffsFacade") then return end

    local db = VUI.db.profile.unitframes.buffs
    ...
    local function ButtonDefault(button)
        local Backdrop = {
            bgFile = nil,
            edgeFile = "Interface\\Addons\\VUI\\Media\\Textures\\Core\\outer_shadow",
            tile = false,
            tileSize = 32,
            edgeSize = 6,
        ...
        border.texture:SetTexture("Interface\\Addons\\VUI\\Media\\Textures\\Core\\gloss_border_w")
        ...
        border.shadow:SetBackdropBorderColor(unpack(VUI:Color(0.25, 0.9)))

        button.VUIBorder = border
    end
    ...
    function updateDebuffs()
        ...
        for index, child in pairs(Children) do
            ...
            if (frame.VUIBorder) then
                ...
                if color ~= nil then
                    frame.VUIBorder.texture:SetVertexColor(color.r, color.g, color.b, 1)
                else
                    frame.VUIBorder.texture:SetVertexColor(unpack(VUI:Color(0.15)))
                end
            end
        end
    end
