local Core = VUI:NewModule("Skins.Core");

function Core:OnEnable()
	local db = VUI.db.profile.general
	local color = VUI:Color()
	...
	if (db and color) then
		...
		VUI:RegisterEvent("GROUP_ROSTER_UPDATE")
		...
		VUI:SetScript("OnEvent", function(self, event)
			ColorRaid()
		...
		CompactRaidFrameManagerToggleButton:SetNormalTexture("Interface\\Addons\\VUI\\Media\\Textures\\RaidFrames\\RaidPanel-Toggle")
		for i = 1, 4 do
