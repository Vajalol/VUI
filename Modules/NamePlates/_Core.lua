...
function Module:OnEnable()
    ...
    local focusTexture = [[Interface\AddOns\VUI\Media\Textures\Nameplates\focusTexture]]
    ...
    for _, npc in pairs(db.npccolors) do
        VUI_NPCColors[npc.id] = npc.color
    end
    ...
    local function iconSkin(icon, parent)
        ...
        border:SetTexture("Interface\\Addons\\VUI\\Media\\Textures\\Core\\gloss")
        border:SetTexCoord(0, 1, 0, 1)
        border:SetDrawLayer("BACKGROUND", -7)
        --border:SetVertexColor(unpack(VUI:Color()))
        ...
        if VUI:Color() then
            back:SetBackdropBorderColor(unpack(VUI:Color(0.25)))
        end
        back:SetAlpha(0.9)
        icon.bg = back
        icon.styled = true
            ...
            if self.healthBar and self.unit then
                if UnitName("player") ~= UnitName(self.unit) then
                    ...
                    if db.colors then
                        ...
                        local color = VUI_NPCColors[tonumber(id)] or { r = 0, g = 1, b = 0.6, a = 1 }
                        local nColor = VUI_NPCColors[tonumber(id)] or { r = 1, g = 0, b = 0.3, a = 1 }
