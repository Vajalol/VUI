local Module = VUI:NewModule("UnitFrames.Target");

function Module:OnEnable()

    local db = {
        unitframes = VUI.db.profile.unitframes,
        texture = VUI.db.profile.general.texture,
        theme = VUI.db.profile.general.theme
    }
        ...
        if db.theme ~= 'Blizzard' then
            if not hooked[aura] then
                ...
                if not aura.border then
                    ...
                    back:SetBackdropBorderColor(unpack(VUI:Color(0.25, 0.9)))
                    aura.bg = back
                end
            end
        end
    ...
    local function VUIColorRepBar(self)
        local reputationBar = self.TargetFrameContent.TargetFrameContentMain.ReputationColor
        reputationBar:SetVertexColor(unpack(VUI:Color(0.15)))
    end
