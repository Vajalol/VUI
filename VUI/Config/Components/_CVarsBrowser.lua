local CvarsBrowser = VUI:NewModule('Config.Components.CvarsBrowser');
...
local function buildBrowser()
    ...
    local browser = VUIConfig:Window(UIParent, 400, 300, 'Cvars Browser')
    ...
    local search = VUIConfig:SearchEditBox(browser, 380, 24, 'Search')
    VUIConfig:GlueBelow(search, browser, 10, 35, 'LEFT')

    local table = VUIConfig:ScrollTable(browser, cols, 14, 14)
    table:EnableSelection(true);
    VUIConfig:GlueTop(table, browser, 0, -60)
