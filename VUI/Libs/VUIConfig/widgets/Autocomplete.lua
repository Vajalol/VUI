...
VUIConfig.Util.autocompleteTransformer = function(_, value)
	return value;
end

VUIConfig.Util.autocompleteValidator = function(self)
	self.VUIConfig:MarkAsValid(self, true);
	return true;
end

VUIConfig.Util.autocompleteItemTransformer = function(_, value)
...
VUIConfig.Util.autocompleteItemValidator = function(ac)
	...
	if itemId then
		ac.value = itemId;
		ac:SetText(itemName);
		self.VUIConfig:MarkAsValid(ac, true);

		return true;
	else
		self.VUIConfig:MarkAsValid(ac, false);
		return false;
	end
end
...
local AutocompleteMethods = {
	...
	buttonCreate = function(panel)
		...
		optionButton = VUIConfig:HighlightButton(panel, panel:GetWidth(), 20, '');
		optionButton.highlight = VUIConfig:HighlightButtonTexture(optionButton);
...
local AutocompleteEvents = {
	...
	OnTextChanged = function(ac, isUserInput)
		local plainText = VUIConfig.Util.stripColors(ac:GetText());
		ac.selectedItem = nil;
...
--- @return EditBox
function VUIConfig:Autocomplete(parent, width, height, text, validator, transformer, items)
	transformer = transformer or VUIConfig.Util.autocompleteTransformer;
	validator = validator or VUIConfig.Util.autocompleteValidator;

	local autocomplete = self:EditBox(parent, width, height, text, validator);
	--@type VUIConfig
	autocomplete.VUIConfig = self;
