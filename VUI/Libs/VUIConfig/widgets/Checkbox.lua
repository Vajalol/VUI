...
--@return CheckButton
function VUIConfig:Checkbox(parent, text, width, height, tooltip)
	local checkbox = CreateFrame('Button', nil, parent);
	checkbox.VUIConfig = self;
	...
	checkbox.target.VUIConfig = self;
	...
	checkbox.checkedTexture = self:Texture(checkbox.target, nil, nil, [[Interface\AddOns\VUI\Libs\VUIConfig\media\Checked]]);
...
function VUIConfig:IconCheckbox(parent, icon, text, width, height, iconSize)
...
--@return CheckButton
function VUIConfig:Radio(parent, text, groupName, width, height)
...
VUIConfig.radioGroups = {};
VUIConfig.radioGroupValues = {};

--@return CheckButton[]
function VUIConfig:RadioGroup(groupName)
...
function VUIConfig:GetRadioGroupValue(groupName)
...
function VUIConfig:SetRadioGroupValue(groupName, value)
	local group = self:RadioGroup(groupName);
...
local radioGroupOnValueChanged = function(radio)
	...
	local newValue = radio.VUIConfig:GetRadioGroupValue(groupName);
	if radio.VUIConfig.radioGroupValues[groupName] ~= newValue then
		radio.OnValueChangedCallback(newValue, groupName);
	end
	radio.VUIConfig.radioGroupValues[groupName] = newValue;
...
function VUIConfig:OnRadioGroupValueChanged(groupName, callback)
	local group = self:RadioGroup(groupName);
...
function VUIConfig:AddToRadioGroup(radio, groupName)
	local group = self:RadioGroup(groupName);
	tinsert(group, radio);
