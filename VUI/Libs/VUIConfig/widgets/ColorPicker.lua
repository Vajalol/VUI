...
--- alphaSliderTexture = [[Interface\AddOns\YourAddon\Libs\VUIConfig\media\Checkers.tga]]
function VUIConfig:ColorPicker(parent, alphaSliderTexture)
	...
	cpf.okButton = VUIConfig:Button(cpf, 100, 20, OKAY);
	cpf.cancelButton = VUIConfig:Button(cpf, 100, 20, CANCEL);
...
-- placeholder
function VUIConfig:ColorPickerFrame(r, g, b, a, okCallback, cancelCallback, alphaSliderTexture, updateCallback)
...
function VUIConfig:ColorInput(parent, label, width, height, color, update, cancel)
	local button = CreateFrame('Button', nil, parent);
	button.VUIConfig = self;
	button:EnableMouse(true);
	self:SetObjSize(button, width, height or 20);
	self:InitWidget(button);

	button.target = self:Panel(button, 16, 16);
	button.target.VUIConfig = self;
	...
	button:SetScript("OnClick", function(self)
		self.VUIConfig:ColorPickerFrame(
			self.color.r,
			self.color.g,
			self.color.b,
			self.color.a,
