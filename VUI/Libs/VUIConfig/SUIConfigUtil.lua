...
--- @param frame Frame
function VUIConfig:MarkAsValid(frame, valid)
...
VUIConfig.Util = {
	...
	moneyBoxValidator   = function(self)
		...
		text = text:trim();
		local total, gold, silver, copper, isValid = VUIConfig.Util.parseMoney(text);

		if not isValid or total == 0 then
			self.VUIConfig:MarkAsValid(self, false);
			return false;
		end

		self:SetText(VUIConfig.Util.formatMoney(total));
		self.value = total;
	...
	moneyBoxValidatorExC   = function(self)
		...
		self:SetText(VUIConfig.Util.formatMoney(total, true));
		self.value = total;
	...
	parseMoney          = function(text)
		text = VUIConfig.Util.stripColors(text);
		local total = 0;
	...
	tableMerge          = function(default, new)
		...
		for k, v in pairs(default) do
			if type(v) == 'table' then
				if new[k] then
					result[k] = VUIConfig.Util.tableMerge(v, new[k]);
				else
					result[k] = v;
				end
			else
	...
	orderedNext         = function(t, state)
		...
		if state == nil then
			...
			t.__orderedIndex = VUIConfig.Util.__genOrderedIndex(t);
	...
	orderedPairs        = function(t)
		return VUIConfig.Util.orderedNext, t, nil;
	end,
