local NPCColors = VUI:NewModule('Config.Components.NPCColors');
...
local function buildWindow()
    ...
    -- Read Data
    local data = VUI.db.profile.nameplates.npccolors

    -- Create Window
    local window = VUIConfig:Window(UIParent, 500, 385, 'NPC Colors')
                ...
                OnClick = function(table, cellFrame, rowFrame, rowData, columnData, rowIndex)
                    VUIConfig:ColorPickerFrame(
                        ...
                        function(cpf)
                            ...
                            table:SetData(data)
                            VUI_NPCColors[rowData.id] = cpf:GetColor()
                        end,
                        function() end,
                        nil,
                        function() end
    ...
    -- Create Table
    local dataTable = VUIConfig:ScrollTable(window, cols, 14, 14)
    dataTable:EnableSelection(true);
    VUIConfig:GlueTop(dataTable, window, 0, -60)

    -- Set Table Data
    dataTable:SetData(data)

    -- Search Box
    local searchBox = VUIConfig:SearchEditBox(window, 458, 24, 'Search')
    VUIConfig:GlueBelow(searchBox, window, 0, 100, 'CENTER')
        ...
        npcInfo.GetNPCInfoByID(
            ...
            function(npc)
                if npc.name then
                    ...
                    VUI_NPCColors[npc.id] = { r = 0, g = 0.55, b = 1, a = 1 }
    ...
    -- Create Input Field
    local input = VUIConfig:NumericBox(window, 150, 24, nil, function() end)
    VUIConfig:GlueBelow(input, window, 0, 40, 'CENTER')
    ...
    -- Input Field Label
    local inputLabel = VUIConfig:Header(window, 'Add NPC by ID', 12)
    VUIConfig:GlueBelow(inputLabel, window, 0, 55, 'CENTER')
end
