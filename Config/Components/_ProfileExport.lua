local ProfileExport = VUI:NewModule('Config.Components.ProfileExport');
...
function ProfileExport:exportProfile(db)
    --AceSerialize
    local serialized_profile = VUI:Serialize(db.profile)
...
local function buildProfileExport()
    ...
    local window = VUIConfig:Window(UIParent, 400, 325, 'Profile Export')
    ...
    local editBox = VUIConfig:MultiLineBox(window, 380, 250, ProfileExport:exportProfile(VUI.db), true)
    ...
    VUIConfig:GlueBelow(closeButton, window, 0, 30)
