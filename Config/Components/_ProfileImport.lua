local ProfileImport = VUI:NewModule('Config.Components.ProfileImport');
...
function ProfileImport:ImportProfile(string, doImport)
    ...
    local valid, imported_Profile = VUI:Deserialize(uncompressed_profile)
    if (doImport) then
        if valid and imported_Profile then
            for i, v in pairs(imported_Profile) do
                if type(v) == "table" then
                    VUI.db.profile[i] = CopyTable(v)
                else
                    local new_value = v
                    VUI.db.profile[i] = new_value
                end
            end
        else
            return "< ERROR: Invalid Import String >"
...
local function buildProfileImport()
    ...
    -- Create Window
    local window = VUIConfig:Window(UIParent, 400, 325, 'Profile Import')
    ...
    -- Create EditBox
    local editBox = VUIConfig:MultiLineBox(window, 380, 250, '< Paste Import String here >')
    VUIConfig:GlueTop(editBox, window, 0, -40)

    editBox.editBox:SetFocus()
    editBox.editBox:HighlightText()

    -- Create Import Button
    local importButton = VUIConfig:Button(window, 65, 20, 'Validate')
    VUIConfig:GlueBelow(importButton, window, -35, 30)

    -- Create Close Button
    local closeButton = VUIConfig:Button(window, 65, 20, 'Close')
    VUIConfig:GlueBelow(closeButton, window, 35, 30)
    ...
    importButton:SetScript("OnClick", function(self)
        ...
        if (ProfileImport:ImportProfile(editBox:GetValue())) then
            ...
            elseif (text == 'Import') then
                ...
                VUIConfig:Confirm('Import Profile', 'This will overwrite your existing settings!', buttons)
            end
        end
    end)
end
