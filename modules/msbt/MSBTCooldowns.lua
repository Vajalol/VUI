...
local function OnUpdateCooldown(cooldownType, cooldownFunc)
        ...
        if (cooldownID) then
                ...
                local _, duration, enabled = cooldownFunc(cooldownID)
                if (enabled == 1) then
                        -- Workaround for Blizzard's API which sometimes reports incorrect cooldown information for death knights.
                ...
                for cooldownType, cooldowns in pairs(activeCooldowns) do
                        ...
                        for cooldownID, remainingDuration in pairs(cooldowns) do
                                ...
                                if (startTime ~= nil) then
                                        ...
                                        -- Workaround for Blizzard's API reporting incorrect cooldown information for pets that have been dismissed.
                                        -- Use an internal timer for pet skills instead of relying on the API.
                                        -- Note: This implementation might need adjustment if skills are added that dynamically adjust pet cooldowns.
