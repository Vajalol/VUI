    ...
    C_Timer.After(0.1, function()
        ...
        if frameType == "Frame" then
            ...
            if name:match("Panel$") then
                ThunderStorm:StyleFrame(frame, {title = name:gsub("Panel$", "")})
            elseif name:match("Frame$") then
                ThunderStorm:StyleFrame(frame)
            end
        ...
        elseif frameType == "StatusBar" then
            ...
            ThunderStorm:StyleStatusBar(frame, options)
        end
    end)
end
...
local function HookStatusBarSetStatusBarTexture(self, texture)
    if self.ThunderStormBarStyled or not texture then return end
    ...
    -- Apply styling
    ThunderStorm:StyleStatusBar(self)
end

-- Apply theme to all existing frames
function ThunderStorm:StyleExistingFrames()
    -- Style main UI frames
    local mainFrames = {
        -- Character frames
    ...
    for _, frameName in ipairs(unitFrames) do
        ...
        if frame and not processedFrames[frame] then
            processedFrames[frame] = true
            self:StyleFrame(frame, {noLightning = true})  -- Avoid lightning effect on unit frames
    ...
    for _, barName in ipairs(statusBars) do
        ...
        if bar and bar.IsObjectType and bar:IsObjectType("StatusBar") and not bar.ThunderStormBarStyled then
            local options = {}
            ...
            elseif barName == "MainMenuExpBar" then
                options.color = {r = 0.2, g = 0.6, b = 0.9, a = 1.0}  -- Custom blue color for XP
                options.border = true
            elseif barName == "ReputationWatchBar" then
                options.color = {r = 0.7, g = 0.5, b = 0.9, a = 1.0}  -- Custom purple color for reputation
                options.border = true
            end
...
function ThunderStorm:Install()
    ...
    eventFrame:SetScript("OnEvent", function(self, event, ...)
        if event == "ADDON_LOADED" or event == "PLAYER_ENTERING_WORLD" then
            ...
            C_Timer.After(1, function()
                ThunderStorm:StyleExistingFrames()
            end)
        end
    end)
    ...
    soundEventFrame:RegisterEvent("LOOT_OPENED")
    soundEventFrame:RegisterEvent("DUEL_REQUESTED")
    soundEventFrame:RegisterEvent("READY_CHECK")
    soundEventFrame:SetScript("OnEvent", function(self, event, ...)
        if event == "PLAYER_LEVEL_UP" then
            -- Play thunder sound on level up
            if ThunderStorm.CustomSounds and ThunderStorm.CustomSounds.Thunder then
                PlaySoundFile(ThunderStorm.CustomSounds.Thunder, "Master")
            end
        end
    end)
end
...
Module:RegisterThemeInstaller("ThunderStorm", ThunderStorm.Install)